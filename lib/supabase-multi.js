...
export const saveIdeaToSupabase = async (idea) => {
  try {
    ...
    if (idea.id) {
      ...
      if (error) throw error;
      console.log('New idea inserted in Supabase');
      return true;
    }
  } catch (err) {
    ...
    if (finance.id) {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          prixUnitaire: finance.prixUnitaire || '',
      ...
      if (error) throw error;
      console.log('New finance item inserted in Supabase');
      return true;
    }
  } catch (err) {
